...
driver.get('https://www.mec.ca/en/products/camping-and-hiking/camping-tents-tarps-and-bivies/tents/c/1449')
time.sleep(2)
...
for i in range(numPages) :
    if i != 0 :
        driver.get(f'https://www.mec.ca/en/products/camping-and-hiking/camping-tents-tarps-and-bivies/tents/c/1449?offset={i * 32}')
        time.sleep(2)
    ...
    for i in range(1, ITEMS_PER_PAGE + MAX_ADS) :
        try :
            ...
            if not productID :
                cursor.execute(f"INSERT INTO products(name, category) VALUES ('{name}', 'Tents');")
                cursor.execute(command)
